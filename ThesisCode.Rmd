...
library(cluster) #linkage methods (hclust to make dendrogram is from the stats package)
library(stats) #dendrogram
library(dendextend) #modify dendrogram
library(factoextra) #modify dendrograms again
library(NbClust)#optimal clusters
...
Lastly, I will summarize my data so that there is one observation of each variable per individual. I should have the following numeric variables for each time frame:
...
8. **Start Date**
```{r}
#Displcement and mean fpt are already summarized. We will need total distance, average turning speed, average speed, time in days
...
Then I will add independent variables
1. **Sex**
2. **Age**
3. **Migration Flyway**
...
OSPR_1hour_summary<-OSPR_1hour_summary[ ,c("trackId", "ID", "FirstDate", "duration", "total_dist", "migrate_speed", "mean_turn_speed", "mean_absolute", "sinuosity", "fpt", "Multiple", "Study", "Migration Year", "Flyway", "Status", "Year Born", "Sex", "Age", "max(location_lat)")] 
...
OSPR_3hour_summary<-OSPR_3hour_summary[ ,c("trackId", "ID", "FirstDate", "duration", "total_dist", "migrate_speed", "mean_turn_speed", "mean_absolute", "sinuosity", "fpt", "Multiple", "Study", "Migration Year", "Flyway", "Status", "Year Born", "Sex", "Age", "max(location_lat)")] 
...
OSPR_1day_summary<-OSPR_1day_summary[ ,c("trackId", "ID", "FirstDate", "duration", "total_dist", "migrate_speed", "mean_turn_speed", "mean_absolute", "sinuosity", "fpt", "Multiple", "Study", "Migration Year", "Flyway", "Status", "Year Born", "Sex", "Age", "max(location_lat)")] 
...
OSPR_3day_summary<-OSPR_3day_summary[ ,c("trackId", "ID", "FirstDate", "duration", "total_dist", "migrate_speed", "mean_turn_speed", "mean_absolute", "sinuosity", "fpt", "Multiple", "Study", "Migration Year", "Flyway", "Status", "Year Born", "Sex", "Age", "max(location_lat)")] 
...
First I will look at the distribution of data for each sampling frequency using histograms, correlation, and PCA
```{r}
...
Are osprey leap-frog migrants?
```{r}
# 1-hour
# Are osprey leap-frog migrants: do birds that migrate further, start further north?
plot(OSPR_1hour_summary$total_dist, OSPR_1hour_summary$Start_Lat)
summary(lm(total_dist ~ Start_Lat, data = OSPR_1hour_summary)) # Apparently not...
plot(OSPR_3hour_summary$total_dist, OSPR_3hour_summary$Start_Lat)
summary(lm(total_dist ~ Start_Lat, data = OSPR_3hour_summary)) #nope
plot(OSPR_1day_summary$total_dist, OSPR_1day_summary$Start_Lat)
summary(lm(total_dist ~ Start_Lat, data = OSPR_1day_summary)) #nope
plot(OSPR_3day_summary$total_dist, OSPR_3day_summary$Start_Lat)
summary(lm(total_dist ~ Start_Lat, data = OSPR_3day_summary)) #nope
#osprey are not leap-frog migrants
```

Do osprey exhibit differential migration between sexes where one sex migrates further than another?
```{r}
#1-hour
...
ggplot(OSPR_1hour_summary_complete, aes(x=Start_Lat, y = total_dist, col = Sex)) +
  geom_point()
boxplot(total_dist~Sex, data = OSPR_1hour_summary_complete)
...
var.test(M_1hour$total_dist, F_1hour$total_dist) #p-value>0.05 by a lot. no siginficant differences between the variances. The distance data seems to be normally distributed from the corr plots above = we can run the t-test
t.test(M_1hour$total_dist, F_1hour$total_dist, var.equal=TRUE) #significant p = 0.022.Total distance traveled by males is different from females. 
mean(F_1hour$total_dist)
mean(M_1hour$total_dist)
#females travel further than males by an average of about 1000 kilometers. 

# 3-hours
OSPR_3hour_summary_complete<-filter(OSPR_3hour_summary, Status !="Incomplete")
ggplot(OSPR_3hour_summary_complete, aes(x=Start_Lat, y = total_dist, col = Sex)) +
  geom_point()
boxplot(total_dist~Sex, data = OSPR_3hour_summary_complete)
#check to make sure variances are homogeneous (assumption of the unpaired two way t-test).
...
var.test(M_3hour$total_dist, F_3hour$total_dist) #p-value>0.05 by a lot. no siginficant differences between the variances. The distance data seems to be normally distributed from the corr plots above = we can run the t-test
t.test(M_3hour$total_dist, F_3hour$total_dist, var.equal=TRUE) #significant p = 0.024.Total distance traveled by males is different from females. 
mean(F_3hour$total_dist)
mean(M_3hour$total_dist)
#females travel further than males by an average of about 900 kilometers.

# 1-day
OSPR_1day_summary_complete<-filter(OSPR_1day_summary, Status !="Incomplete")
ggplot(OSPR_1day_summary_complete, aes(x=Start_Lat, y = total_dist, col = Sex)) +
  geom_point()
boxplot(total_dist~Sex, data = OSPR_1day_summary_complete)
#check to make sure variances are homogeneous (assumption of the unpaired two way t-test).
...
var.test(M_1day$total_dist, F_1day$total_dist) #p-value>0.05 by a lot. no siginficant differences between the variances. The distance data seems to be normally distributed, but less so than from the other two sampling periods from the corr plots above = we can run the t-test
t.test(M_1day$total_dist, F_1day$total_dist, var.equal=TRUE) #significant p = 0.023.Total distance traveled by males is different from females. 
mean(F_1day$total_dist)
mean(M_1day$total_dist)
...
ggplot(OSPR_3day_summary_complete, aes(x=Start_Lat, y = total_dist, col = Sex)) +
  geom_point()
boxplot(total_dist~Sex, data = OSPR_3day_summary_complete)
#check to make sure variances are homogeneous (assumption of the unpaired two way t-test).
...
var.test(M_3day$total_dist, F_3day$total_dist) #p-value>0.05 by a lot, however the heteroscedasticity is becoming more apparent = the variances between the two data sets are becoming more different, yet there are still no significant differences between the variances. The distance data seems to be normally distributed, but less so than from the other two sampling periods from the corr plots above = we can run the t-test
t.test(M_3day$total_dist, F_3day$total_dist, var.equal=TRUE) #significant p = 0.022.Total distance traveled by males is different from females. 
mean(F_3day$total_dist)
mean(M_3day$total_dist)
...
```{r}
#1-hour
ggplot(OSPR_1hour_summary_complete, aes(x=Start_Lat, y = total_dist, col = Age)) +
  geom_point()
boxplot(total_dist~Age, data = OSPR_1hour_summary_complete)
...
var.test(J_1hour$total_dist, A_1hour$total_dist) #no difference
t.test(J_1hour$total_dist, A_1hour$total_dist, var.equal=TRUE) #There is a significant difference (birds at higher latitudes where juveniles will fly less distance than adults)

#3-hour
ggplot(OSPR_3hour_summary_complete, aes(x=Start_Lat, y = total_dist, col = Age)) +
  geom_point()
boxplot(total_dist~Age, data = OSPR_3hour_summary_complete)
...
var.test(J_3hour$total_dist, A_3hour$total_dist) #no difference
t.test(J_3hour$total_dist, A_3hour$total_dist, var.equal=TRUE) #There is a significant difference (birds at higher latitudes where juveniles will fly less distance than adults)

#1-day
ggplot(OSPR_1day_summary_complete, aes(x=Start_Lat, y = total_dist, col = Age)) +
  geom_point()
boxplot(total_dist~Age, data = OSPR_1day_summary_complete)
...
var.test(J_1day$total_dist, A_1day$total_dist) #no difference
t.test(J_1day$total_dist, A_1day$total_dist, var.equal=TRUE) #There is a significant difference (birds at higher latitudes where juveniles will fly less distance than adults)

#3-day
ggplot(OSPR_3day_summary_complete, aes(x=Start_Lat, y = total_dist, col = Age)) +
  geom_point()
boxplot(total_dist~Age, data = OSPR_3day_summary_complete)
...
var.test(J_3day$total_dist, A_3day$total_dist) #no difference
t.test(J_3day$total_dist, A_3day$total_dist, var.equal=TRUE) #significant difference

#Conclusion: I saw a significant difference between the distance traveled by juveniles compared to adults. Juveniles supposedly traveled less distance than adults, however I do not think these results reflect the biology of osprey. All male osprey starting at higher latitudes (where differentiation between sexes occurs) were juvenile while all females were adults. This means that my data actually reflects sexual differentiation rather than age. When looking at lower and mid latitudes, juveniles were evenly mixed with adults. Finally, my p-values for distance ~ age were similar suggesting that sex may have skewed my results.
...
Migration strategies should be impacted by the distance the bird flies, the sex, and the age of the bird. I do not have enough data on origin of the birds to make any conclusions. 
```{r}
#Hypothesis: Females and males will exhibit different migration strategies since females travel around 1000 kilometers more on average than males.
#migration strategy will not include distance as a characteristic anymore as distance is thought to influence strategy. It is now an independent variable. 

#hmmmm not seeing much correlation here between latitude and total distance but sex does seem to be an influence. Lets add some more variables including sex, age, latitude, Birth_Year, Flyway, Migration_Year, and Origin

OSPR_1hour_summary_filter<-filter(OSPR_1hour_summary, Sex != "Unkown") #139 migration events
OSPR_1hour_summary_filter<-filter(OSPR_1hour_summary_filter, Age != "Unkown")
Anova(lm(total_dist~Start_Lat + Sex + Age + Birth_Year + Flyway + Migration_Year + Origin, data = OSPR_1hour_summary_Sex)) #migration year, age, and sex seem to be the influencing factors. 

#But what about the relationship between these independent variables and the fact that my data is non-normally distributed? What is the relationship of my variables with the rest of the dependent variables? We need a multivariate non-parametric test to answer:
  # 1. Which independent variables show distinct strategies for each level?
    # * Does strategy differ between age, sex, flyway, starting Latitude, origin, or Migration_Year, or
        # Birth_Year?
  # 2. Which characteristics contribute to differences between groups?




dis_1hour <- vegdist(OSPR_1hour_summary_filter[,4:10])
mod <- betadisper(dis_1hour, OSP1hrSum$k_3, type = "centroid")
anova(mod) #does not meet assumptions and design is unbalanced
plot(mod) #largest group has the most dispersion = PERMANOVA test will be overly conservative
adonis2(dis_1hour ~ OSP1hrSum$k_3, method = "euclidean") #we still see a significant result so the groups differ in location as well as dispersion. 
TukeyHSD(mod) #no diff between class 1 and 2, but differences between 1 and 3 and 2 and 3. 2 may be the optimum number of clusters
```


```{r}
# 3-hour
dis_3hour <- vegdist(OSP3hrSum[,1:7])
mod2 <- betadisper(dis_3hour, OSP3hrSum$k_2, type = "centroid")
anova(mod2) #meets assumptions. Variances are equally distributed
plot(mod2) #visually they don't look that different
adonis2(dis_1hour ~ OSP3hrSum$k_2, method = "euclidean") #No difference between the groups
```
...
ac <- function(x) {
  agnes(OSPR_1hour_norm, method = x)$ac
}
sapply(m, ac) #ward is best for identifying strongest clustering structure.

#optimal clusters
NbClust(OSPR_1hour_norm, method = "ward.D2", index = "all")$Best.nc # 3 clusters

#create a new osprey summary object and assign color schemes the arrange
OSP1hrSum<-OSPR_1hour_summary
OSP1hrSum<-cbind(OSPR_1hour_norm, OSP1hrSum[,c(1:3, 11:19)])
OSPR_1hour_clust <- OSP1hrSum[,1:7] %>% dist %>% hclust(method = "ward.D2") %>% as.dendrogram 
k_3 <- cutree(OSPR_1hour_clust, k = 3, order_clusters_as_data = TRUE)
OSP1hrSum<-cbind(OSP1hrSum, k_3)
oldvals3 <- unique(OSP1hrSum$k_3)
newvals3 <- c("#33D1DE", "#2C2AAF", "#6FB568")
OSP1hrSum$ClassColors<-newvals3[ match(OSP1hrSum$k_3, oldvals3) ]
oldvals <- unique(OSP1hrSum$Flyway)
newvals <- c("#33D1DE", "#2C2AAF", "#6FB568")
oldvals2 <- unique(OSP1hrSum$Sex)
newvals2 <- c("Blue", "Red", "Gray")
OSP1hrSum$FlywayColors<-newvals[ match(OSP1hrSum$Flyway, oldvals) ]
OSP1hrSum$SexColors<-newvals2[ match(OSP1hrSum$Sex, oldvals2) ]
OSP1hrSum<-OSP1hrSum %>% arrange(k_3) #rearrange so labels are correct on dendrogram

OSPR_1hour_clust %>%
  set("branches_k_color", k = 3, value = c("#FF0000", "#E462C7", "#F5D000")) %>%
  set("labels", OSP1hrSum$trackId) %>% #set labels
  set("labels_cex", 0.3) %>%        # change label size
  set("labels_col", c(OSP1hrSum$FlywayColors)) %>% #change label color
  plot()
```
...
names(m) <- c( "average", "single", "complete", "ward")
ac <- function(x) {
  agnes(OSPR_3hour_norm, method = x)$ac
}
sapply(m, ac) #ward

OSPR_3hour_clust<-hclust(dist(OSPR_3hour_norm), method = "ward.D2")

#optimal clusters
NbClust(OSPR_3hour_norm, method = "ward.D2", index = "all")$Best.nc # 2 clusters

OSP3hrSum<-OSPR_3hour_summary
OSP3hrSum<-cbind(OSPR_3hour_norm, OSP3hrSum[,c(1:3, 11:19)])
OSPR_3hour_clust <- OSP3hrSum[,1:7] %>% dist %>% hclust(method = "ward.D2") %>% as.dendrogram 
k_2 <- cutree(OSPR_3hour_clust, k = 2, order_clusters_as_data = TRUE)
OSP3hrSum<-cbind(OSP3hrSum, k_2)
oldvals3 <- unique(OSP3hrSum$k_2)
newvals3 <- c("#33D1DE", "#2C2AAF")
OSP3hrSum$ClassColors<-newvals3[ match(OSP3hrSum$k_2, oldvals3) ]
oldvals <- unique(OSP3hrSum$Flyway)
newvals <- c("#33D1DE", "#2C2AAF", "#6FB568")
oldvals2 <- unique(OSP3hrSum$Sex)
newvals2 <- c("Blue", "Red", "Gray")
OSP3hrSum$FlywayColors<-newvals[ match(OSP3hrSum$Flyway, oldvals) ]
OSP3hrSum$SexColors<-newvals2[ match(OSP3hrSum$Sex, oldvals2) ]
OSP3hrSum<-OSP1hrSum %>% arrange(k_3) #rearrange so labels are correct on dendrogram

OSPR_3hour_clust %>%
  set("branches_k_color", k = 2, value = c("#FF0000", "#E462C7")) %>%
  set("labels", OSP3hrSum$trackId) %>% #set labels
  set("labels_cex", 0.3) %>%        # change label size
  set("labels_col", c(OSP3hrSum$FlywayColors)) %>% #change label color
  plot()


dl <- dendlist(OSPR_1hour_clust, OSPR_3hour_clust)
?tanglegram
tanglegram(dl, sort = TRUE, common_subtrees_color_lines = T, highlight_distinct_edges  = FALSE, highlight_branches_lwd = FALSE, main_left = "1-hour", main_right = "3-hour", margin_inner = 0.5)
```
...
names(m) <- c( "average", "single", "complete", "ward")
ac <- function(x) {
  agnes(OSPR_1day_norm, method = x)$ac
}
sapply(m, ac) #ward

OSPR_1day_clust<-hclust(dist(OSPR_1day_norm), method = "ward.D2")

#optimal clusters
NbClust(OSPR_1day_norm, method = "ward.D2", index = "all")$Best.nc # 3 clusters

fviz_dend(x = OSPR_1day_clust, lwd = 0.8, k = 3,
          show_labels = F,
          k_colors = c("#117733", "#DDCC77", "#CC6677"),
          rect = TRUE, 
          rect_border = c("#117733", "#DDCC77", "#CC6677"), 
          rect_fill = T,
          horiz = T)
```
...
names(m) <- c( "average", "single", "complete", "ward")
ac <- function(x) {
  agnes(OSPR_3day_norm, method = x)$ac
}
sapply(m, ac) #ward

OSPR_3day_clust<-hclust(dist(OSPR_3day_norm), method = "ward.D2")

NbClust(OSPR_3day_norm, method = "ward.D2", index = "all")$Best.nc # 3 clusters

fviz_dend(x = OSPR_3day_clust, lwd = 0.8, k = 3,
          show_labels = F,
          k_colors = c("#117733", "#DDCC77", "#CC6677"),
          rect = TRUE, 
          rect_border = c("#117733", "#DDCC77", "#CC6677"), 
