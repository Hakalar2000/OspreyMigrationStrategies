...
I predict migration strategies should be impacted by the distance the bird flies, the sex, and the age of the bird. However, I do not have enough data on origin of the birds to make any conclusions and the sexes and age data is very skewed. Instead of attempting to determine the relationship between groups, I can use HCA to determine the relationship between individuals and use a heat map to compare the differences in demographics, geographic origin, and migratory characteristics to get a better idea of how osprey with more similar strategies relate to one another.  
...
OSPR_1hour_norm<-cbind(OSPR_1hour_norm, OSPR_1hour_summary[,c(1:4, 12:19)])
...
annotation<-rowAnnotation(df = OSPR_1hour_norm[,13:19])
...
anova(mod) #does not meet assumptions and design is unbalanced. Dispersions are signficantly different
...
boxplot(OSPR_1hour_summary$cluster, OSPR_1hour_summary$duration)
```
