...
IL_OspreyData<-IL_Osprey_Data[IL_Osprey_Data$ID %in% keep, ] 
```
...
#arrange and clean data format
df %>%
  group_by(migrationEvent) %>%
  arrange(desc(ymd_hms(df$timestamp)))
names(df) [names(df) == "migrationEvent"] <- "individual.local.identifier"
```
...
OSPR<-df2move(df,
        ...
        time = "timestamp", track_id = "individual.local.identifier")
