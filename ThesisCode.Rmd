...
> **Do strategies vary between and within individuals?**
> **Do ospreys use the same migration strategies between regions?**
> **How does changing the temporal sampling frequency affect the results of my analysis?**

> This document was created using R Markdown and will not cover the nuances I used to code the Markdown file (if you want to access the full Markdown script, you can view the file on my Github). For simplicity and privacy, not all code lines are shown in the Rmarkdown output when downloading data. I have done my best to annotate my code for easy interpretation, but this is not a guide on how to use R. Some aspects of coding will *not* be explained.
...
#lapply function applies a function to a list. Each individual within the data set should have a median or mode timelag of 1 hour or finer. I used median/mode instead of mean since most studies only have trackers turned on during daylight hours. This means that  mean lag values will be larger than 1 hour. 
...
#remove outliers
#filter erroneous points using quantiative methods: Erroneous points are those with unrealistically high speed and turning angle (course). For course (often referred to and confused with heading) we will look at the 90th percentile of points as suggested by Gupte et al., 2021. Course is measured as the angle from magnetic north (clockwise) in which the tracker is moving. For speed I used Kerlinger 1989 maximum recorded flight speed for ospreys which is 33.4 meters/second.
...
length(unique(segs$SegmentID)) #48 segments total
```
...
mean(fpt_max_subset_1hour[,1]) #11 km
...
Subset3hour %>%
  ...
  summarise(m_speed = mean(speed),
            m_turn_speed = mean(turn_speed),
            m_abs_angle = mean (abs_angle),
            m_FPT = mean(FPT))
...
min_Subset1hour<-Subset1hour %>%
  ...
  summarise(N = n()) %>%
  as.data.frame()
min(min_Subset1hour$N) #23 random samples needed
...
min_Subset3hour<-Subset3hour %>%
  ...
  summarise(N = n()) %>%
  as.data.frame()
min(min_Subset3hour$N) #8 random samples
...
table(Norm_1hour$migrationEvent)
...
```{r}
#1-hour
colors_Events<-colorRampPalette(c("#E0AA20", "#ECC713", "#F1E306", "#17E48B","#1BA554", "#268E74", "#0B68FF", "#182DD4", "#073C73", "#39056F"))(16)
colors_Ind<-colorRampPalette(c("#E0AA20", "#ECC713", "#F1E306", "#17E48B","#1BA554", "#268E74", "#0B68FF", "#182DD4", "#073C73", "#39056F"))(8)
...
fviz_pca_var(EventsPCA_3hour,
             col.var =  "cos2", arrowsize = 1.5, label = F,
             title = " ") +
  theme(legend.position = "none", 
        axis.title.x = element_text(margin = margin(t = 30), size = 15),
        axis.title.y = element_text(margin = margin(r = 30), size = 15),
...
permutest(mod_event)
permutest(mod_Segment)
permutest(mod_Ind) #Not significant = Individuals do not have different dispersions
...
dispersions_segment<-TukeyHSD(mod_Segment)
dispersions_m_segment<-tukey_to_matrix(dispersions_segment$group) #make into a matrix for corrplot
dispersions_m_segment[is.na(dispersions_m_segment)]<- 1
dispersions_m_segment_t<-t(dispersions_m_segment)
dispersions_m_segment_t[is.na(dispersions_m_segment_t)]<- 1
dispersions_segment_1hr<-dispersions_m_segment*dispersions_m_segment_t
...
dispersions_segment_1hr
Heatmap(dispersions_segment_1hr, name = "p-value", 
...
#Test the homogeneity of dispersion between groups. If the p-value is > 0.05, then the assumptions of PERMANOVA are met: the null hypothesis is that overall dispersion between groups is not different.
permutest(mod_event_3)
permutest(mod_Segment_3)
permutest(mod_Ind_3)
```
...
#Test the homogeneity of dispersion between groups. If the p-value is > 0.05, then the assumptions of PERMANOVA are met: the null hypothesis is that overall dispersion between groups is not different.
permutest(mod_event_24)
permutest(mod_Segment_24)
permutest(mod_Ind_24)
...
dispersions_Ind_24<-TukeyHSD(mod_Ind_24)
dispersions_m_Ind_24<-tukey_to_matrix(dispersions_Ind_24$group) #make into a matrix for corrplot
dispersions_m_Ind_24[is.na(dispersions_m_Ind_24)]<- 1
dispersions_m_Ind_24_t<-t(dispersions_m_Ind_24)
dispersions_m_Ind_24_t[is.na(dispersions_m_Ind_24_t)]<- 1
dispersions_Ind_24hr<-dispersions_m_Ind_24*dispersions_m_Ind_24_t
...
Heatmap(dispersions_Ind_24hr, name = "p-value", 
        row_names_gp = gpar(fontsize = 15),
        column_names_gp= gpar(fontsize = 15),
        cluster_rows = F,
        cluster_columns = F,
...
pairperm_segment3 #report as table
```
...
pairperm_segment24 #report as table
```
