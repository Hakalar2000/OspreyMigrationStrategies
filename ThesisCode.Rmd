...
# What if we added sex as a level since female birds are known to migrate further than males? We can do this via a multiple linear regression model.
ggplot(OSPR_1hour_summary, aes(x=Start_Lat, y = total_dist, col = Sex)) +
  geom_point()
boxplot(total_dist~Sex, data = OSP1hrSum[,8:26])
OSPR_1hour_summary_Sex<-filter(OSPR_1hour_summary, Sex != "Unkown")
Anova(lm(total_dist~Start_Lat + Sex, data = OSPR_1hour_summary_Sex))
