...
library(janitor) #summarizing characteristics for each dendrogram cluster
...
#merge dataframes and keep only the needed columns and rename the columns
...
```{r}
#normality of numeric data using Hmisc package
hist.data.frame(OSPR_1hour_summary[,4:10]) 
hist.data.frame(OSPR_3hour_summary[,4:10])
hist.data.frame(OSPR_1day_summary[,4:10])
hist.data.frame(OSPR_3day_summary[,4:10])
#only absolute angle is somewhat normally distributed. 

#Next lets look at correlation between variables
plot(OSPR_1hour_summary[,4:10])
cor(OSPR_1hour_summary[,4:10], method = "spearman")
#total distance and duration; sinuosity and duration seem to be correlated
plot(OSPR_3hour_summary[,4:10])
cor(OSPR_3hour_summary[,4:10], method = "spearman") #duration and total distance; sinuosity and duration correlated
plot(OSPR_1day_summary[,4:10])
cor(OSPR_1day_summary[,4:10], method = "spearman")#duration and total distance; duration and fpt more correlated and duration and sinuosity less correlated. 
plot(OSPR_3day_summary[,4:10])
cor(OSPR_3day_summary[,4:10], method = "spearman", use = "complete.obs") #fpt, speed, distance all correlated with duration. Note missing values for some birds. These birds cannot be included in the HCA. 
OSPR_3day_summary<-na.omit(OSPR_3day_summary)

#normalize data between 0-1. Needed for HCA and Hopkins statistic
OSPR_1hour_norm<-as.data.frame(lapply
                               (OSPR_1hour_summary[,4:10], min_max_norm))
OSPR_3hour_norm<-as.data.frame(lapply
                               (OSPR_3hour_summary[,4:10], min_max_norm))
OSPR_1day_norm<-as.data.frame(lapply
                               (OSPR_1day_summary[,4:10], min_max_norm))
OSPR_3day_norm<-as.data.frame(lapply
                               (OSPR_3day_summary[,4:10], min_max_norm))

#clustering potential of data
hopkins(OSPR_1hour_norm) # 0.9999~
hopkins(OSPR_3hour_norm) # 0.9996
hopkins(OSPR_1day_norm) #0.999
hopkins(OSPR_3day_norm) #0.999
#all of the data has high clustering potential so it is not uniformly distributed since the values are close to 1. No need for a PCA.
```
...
# 1-hour
str(OSP1hrSum)
dis_1hour <- vegdist(OSP1hrSum[,1:7])
